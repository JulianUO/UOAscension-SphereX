...
bool CChar::Skill_MakeItem( ITEMID_TYPE id, CUID uidTarg, SKTRIG_TYPE stage, bool fSkillOnly, int iReplicationQty )
{
	...
	{
		m_atCreate.m_Amount = (word)(iReplicationQty); // how much resources we really consumed
...
bool CChar::Skill_Mining_Smelt( CItem * pItemOre, CItem * pItemTarg )
{
	...
	UpdateDir( m_Act_p );
	if ( pItemOre->IsAttr(ATTR_MAGIC|ATTR_BLESSED))	// not magic items
	{
...
int CChar::Skill_Poisoning( SKTRIG_TYPE stage )
{
	...
	switch ( pItem->GetType() )
	{
		...
		case IT_FOOD_RAW:
		case IT_MEAT_RAW:
			pItem->m_itFood.m_poison_skill = (byte)(pPoison->m_itPotion.m_skillquality / 10);
			break;
		case IT_WEAPON_MACE_SHARP:
		case IT_WEAPON_SWORD:		// 13 =
		case IT_WEAPON_FENCE:		// 14 = can't be used to chop trees. (make kindling)
			pItem->m_itWeapon.m_poison_skill = (byte)(pPoison->m_itPotion.m_skillquality / 10);
